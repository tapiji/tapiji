...
public final class PropertiesGenerator {
     ...
     * @param iBundle the bundle used to generate the string
     * @return the generated string
     */
    public static String generate(IBundle iBundle) {
        String lineBreak = SYSTEM_LINE_SEP;
        int numOfLineBreaks = RBEPreferences.getGroupLineBreaks();
        StringBuffer text = new StringBuffer();

        // Header comment
        String headComment = iBundle.getComment();
        ...
        String group = null;
        int equalIndex = -1;
        for (Iterator iter = iBundle.getKeys().iterator(); iter.hasNext();) {
            IBundleEntry bundleEntry = (IBundleEntry) iBundle.getEntry((String) iter.next());
            ...
            if (RBEPreferences.getKeepEmptyFields() || value.length() > 0) {
                ...
                if (RBEPreferences.getGroupKeys()) {
                    ...
                    if (newGroup == null || !newGroup.equals(group)) {
                        group = newGroup;
                        equalIndex = getEqualIndex(key, group, iBundle);
                        for (int i = 0; i < numOfLineBreaks; i++) {
                            text.append(lineBreak);
                        }
                    }
                } else {
                    equalIndex = getEqualIndex(key, null, iBundle);
                }
...
//        text.append(key);
     ...
     * @param iBundle resource bundle
     * @return position
     */
    private static int getEqualIndex(String key, String group, IBundle iBundle) {
        int equalIndex = -1;
