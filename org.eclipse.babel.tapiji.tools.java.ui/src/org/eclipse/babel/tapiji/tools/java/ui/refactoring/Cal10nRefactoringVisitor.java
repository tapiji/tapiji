...
/**
 * Refactors java files, which are using the old enumeration key
 * that gets refactored.
 * 
...
public class Cal10nRefactoringVisitor extends ASTVisitor {
	 ...
	 * @param enumPath The path of the enum file to change
	 * @param cu The {@link CompilationUnit} to modify
	 * @param changeSet The set of changes (protocol)
	 */
	public Cal10nRefactoringVisitor(CompilationUnit cu, 
			String oldKey, String newKey, String enumPath, List<String> changeSet) {
		this.oldKey = oldKey;
		this.newKey = newKey;
	...
	/**
	 * Changes the argument of the IMessageConveyor#getMessage(...) call.
	 * The new enum key replaces the old one.
	 */
	...
	public boolean visit(MethodInvocation node) {
		...
		if (!isCal10nCall) {
			...
			String fullPath = qName.resolveTypeBinding().getJavaElement().getResource().getFullPath().toPortableString();
			if (fullPath.equals(enumPath) && qName.getName().toString().equals(oldKey)) {
                ...
                Name newName = ast.newName(qName.getQualifier().getFullyQualifiedName());
                QualifiedName newQualifiedName = ast.newQualifiedName(newName, newSimpleName);
