package com.gknsintermetals.eclipse.resourcebundle.manager.auditor;
...
import com.gknsintermetals.eclipse.resourcebundle.manager.auditor.quickfix.MissingLanguageResolution;
...
public class ResourceBundleAuditor extends I18nRBAuditor {
	...
	public static boolean AUDIT_SAME_VALUES = false;
	...
	public  void audit(IResource resource) {		
		if (!RBFileUtils.checkIsResourceBundleFile(resource)) return;
	...
	public void audit(String rbId, ResourceBundleManager rbmanager){
		...
		for (IResource r : bundlefile){
			...
			for (String key: keys){
				...
				if (auditUnspecifiedKey(f1, key, bundlegroup)) /*do nothing - all just done*/;
				else {
					
					if (AUDIT_SAME_VALUES)
		...
		auditMissingLanguage(rbLocales, projectLocales,rbmanager, rbId); 	//projectLocales.size() >= rbLocales.size() -> "Add fragment support"
	}

	/*
	 * Audit the file if the key is not specified. If the value is null report a problem.
	 */
	private boolean auditUnspecifiedKey(IFile f1, String key, IBundleGroup bundlegroup) {
	...
	/*
	 * Compare a key in different files and report a problem if the value is the same.
	 */
	private void auditSameValues(IFile f1, IFile f2, String key, IBundleGroup bundlegroup){
		...
		if (!l2.equals(l1)){
			...
			if (bundleentry != null)
				if (bundlegroup.getBundleEntry(l1, key).getValue().equals(bundleentry.getValue())){
					int pos1 = calculateKeyLine(key,f1);
					int pos2 = calculateKeyLine(key,f2);
					sameValues.put( new RBLocation(f1, pos1, pos1+1, key), new RBLocation(f2, pos2, pos2+1, key));
				}
		}
	}
	
	/*
	 * check if the resourcebundle supports all project-languages and report missing languages.
	 */
	private void auditMissingLanguage(Set<Locale> rbLocales, Set<Locale> projectLocales, ResourceBundleManager rbmanager, String rbId) {
		for (Locale pLocale : projectLocales) {
			if (!rbLocales.contains(pLocale)) {
				...
				IResource representative=rbmanager.getResourceBundleFile(rbId, new Locale(""));
				if (representative == null)
					representative = rbmanager.getRandomFile(rbId);
				missingLanguages.add(new RBLocation((IFile) representative, 1, 2, language));
			}
		}
	}
	...
	private int calculateKeyLine(String key, IFile file){
	    ...
	    { 	
	    	...
	    	while((line = bf.readLine()) != null){
				if ((!line.isEmpty()) && (!line.startsWith("#")) && (line.compareTo(key) > 0)) 
					return linenumber;
				linenumber++;
			}
	...
	public List<IMarkerResolution> getMarkerResolutions(IMarker marker) {
		...
		switch (marker.getAttribute("cause", -1)) {
			case IMarkerConstants.CAUSE_MISSING_LANGUAGE:
				Locale l = new Locale(marker.getAttribute("bundleName",""));		//TODO change Name
