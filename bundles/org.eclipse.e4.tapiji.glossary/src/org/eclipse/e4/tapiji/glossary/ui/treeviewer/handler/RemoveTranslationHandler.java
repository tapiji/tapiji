...
import org.eclipse.e4.tapiji.glossary.ui.dialog.CheckedTreeSelectionDialog;
import org.eclipse.e4.tapiji.glossary.ui.treeviewer.handler.provider.LocaleContentProvider;
import org.eclipse.e4.tapiji.glossary.ui.treeviewer.handler.provider.LocaleLabelProvider;
...
public final class RemoveTranslationHandler {
	...
	public void execute(@Optional @Named(IServiceConstants.ACTIVE_SELECTION) final Term term,
		...
		final CheckedTreeSelectionDialog localeDialog = new CheckedTreeSelectionDialog(shell, new LocaleLabelProvider(),
				new LocaleContentProvider());

		localeDialog.setInput(
	...
	public boolean canExecute(final IGlossaryService glossaryService) {
		if ((null == glossaryService.getGlossary()) || (glossaryService.getTranslations().length == 1)
				|| (glossaryService.getTranslations().length == 0)) {
			return false;
		}
		return true;
	}
