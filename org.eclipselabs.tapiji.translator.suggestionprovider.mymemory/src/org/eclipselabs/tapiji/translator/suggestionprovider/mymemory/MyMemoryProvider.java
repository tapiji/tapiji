...
public class MyMemoryProvider implements ISuggestionProvider {
	...
	private static final String QUOTA_EXCEEDED =
			"YOU USED ALL AVAILABLE FREE TRANSLATION FOR TODAY";
	...
	private static final Logger LOGGER = Logger
			.getLogger(MyMemoryProvider.class.getName());
	...
	public MyMemoryProvider() {
		...
		configSettings = 
				new HashMap<String, ISuggestionProviderConfigurationSetting>();
	...
	private void setSourceLanguage(String lang) {
		if (lang != null) {
			...
			LOGGER.log(LOG_LEVEL, "No source language" + " has benn set, "
	 ...
	 * English to {@code targetLanguage}, then returns translation as Suggestion
	 * object
	 * 
	 * @param original
	 *            is the original text that is going be translated.
	 * @param targetLanguage
	 *            should be in ISO 639-1 Code, e.g "de" for GERMAN.
	...
	public Suggestion getSuggestion(String original, String targetLanguage) {
		...
		// REST GET
		String langpair = SOURCE_LANG
				+ targetLanguage.toLowerCase().subSequence(0, 2);
		...
		String translatedText = jobject.get("translatedText").toString()
				.replaceAll("\"", "");
	...
	public boolean equals(Object obj) {
		if (this == obj) {
			...
			MyMemoryProvider mmp = (MyMemoryProvider) obj;
